...
@frappe.whitelist()
def category_sync():
    ensure_updated_cache(True, True)
    print("updated_cache done")
    copy_cats()
    print("copy cats done")
...
def create_ebay_categories_cache(categories_data):
    ...
    try:
        ...
        print("issue with MinReserve price?")
        raise
        ...
        while cat_children:
            ...
            for cat_child in cat_children:
                ...
                try:
                    ...
                    print("data too long error?", cat_child)
                    raise
        ...
        for i_search, search_function in enumerate(search_functions):
            ...
            if not search_multiple_elements[i_search]:
                ...
                if None not in results[i_search].values():
                    search_done[i_search] = True
            elif (not isinstance(result, basestring) and isinstance(result, collections.Sequence)):
                # Multiple results; treat each element separately
...
##### Temporary workaround - copy records from eBay_categories_hierachy to Item Group eBay
...
def copy_cats():
    ...
    records = frappe.db.sql("""
        select categoryName, categoryID, categoryParentID 
        from eBay_categories_hierarchy
    """, as_dict=1)

    for r in records:
        parent_cat = category_parent_search(r.categoryParentID)
        cat_id = r.categoryID

        if parent_cat:
            cat = str(cat_id) + ' ' + parent_cat.encode('ascii', errors='ignore')  + ' | ' + r.categoryName.encode('ascii', errors='ignore')
        else:
            cat = str(cat_id) + ' ' + r.categoryName.encode('ascii', errors='ignore')

        print(cat)

        ige = frappe.get_doc({
                "doctype": "Item Group eBay",
                "ebay_category_id": str(cat_id),
                "ebay_category": cat.encode('ascii', errors='ignore')
                })

        val = ige.insert(ignore_permissions=True)



def category_parent_search(parent_id):
